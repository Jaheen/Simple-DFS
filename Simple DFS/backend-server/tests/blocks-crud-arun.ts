...
function uploadBlocks(filepath: string) {
   ...
   filestream.once("end", () => {
      axios
         .post("http://192.168.43.202:6789/storage/uploadBlocks", formData, {
...
function downloadBlocks(blockIdentifiers: string[]) {
   ...
   axios
      .get(
         `http://192.168.43.202:6789/storage/getBlocks/${JSON.stringify(
            blockIdentifiers
         )}`,
...
function deleteBlocks(blockIdentifiers: string[]) {
   axios.delete(`http://192.168.43.202:6789/storage/deleteBlocks/${JSON.stringify(blockIdentifiers)}`, {
      headers: {
